...
public class PeriodoAcademicoServiceImpl implements PeriodoAcademicoService {
    ...
    public CambioEstadoResponse cerrarFormulario(Long periodoId) {
        ...
        if (formId == null || formId.isEmpty()) {
            formId = periodo.getFormId();
        }

        if (formId == null || formId.isEmpty()) {
            // Si no tenemos ID del formulario, no podemos cerrarlo en Google, pero
            // permitimos cerrar el periodo en el sistema para continuar manualmente
            log.warn("No se pudo obtener el FormID. Se forzará el cierre del periodo sin contactar a Google.");
        } else {
            try {
                ...
                if (datos == null || datos.isEmpty()) {
                    // CORRECCIÓN: NO lanzamos excepción, solo advertimos.
                    log.warn("El formulario se cerró en Google, pero no se encontraron respuestas automáticas. Se requiere carga manual.");
                } else {
                    ...
                    log.info("Se procesaron {} respuestas automáticas.", datos.size());
                }

            } catch (GoogleFormsException ex) {
                log.error("Fallo en conexión con Google Forms: {}", ex.getMessage());
                // Continuamos para cerrar el periodo localmente
            } catch (Exception ex) {
                log.error("Error inesperado al cerrar formulario en Google: {}", ex.getMessage());
                // Continuamos para cerrar el periodo localmente
            }
        }
    ...
    /**
     * Extrae el formId soportando múltiples formatos de URL
     */
    private String extraerFormId(String urlFormulario) {
        ...
        try {
            if (urlFormulario.contains("/d/e/")) {
                String[] partes = urlFormulario.split("/d/e/");
                if (partes.length > 1) return partes[1].split("/")[0];
            }
            if (urlFormulario.contains("/d/")) {
                String[] partes = urlFormulario.split("/d/");
                if (partes.length > 1) return partes[1].split("/")[0];
            }
            return null;
        } catch (Exception e) {
            log.warn("No se pudo extraer el formId de la URL: {}", urlFormulario);
            return null;
        }
    }
