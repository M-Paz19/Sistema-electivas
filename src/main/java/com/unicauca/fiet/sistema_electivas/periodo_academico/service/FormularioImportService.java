        ...
        for (Map<String, String> datos : datosCrudos) {
            ...
            if (ts != null && !ts.isBlank()) {
                try {
                    ...
                    fechaRespuesta = zdt.toInstant();
                } catch (DateTimeParseException e) {
                    try {
                        fechaRespuesta = Instant.parse(ts); // Intento ISO-8601 directo
                    } catch (Exception ex) {
                        log.warn("No se pudo parsear la fecha '{}', se usa Instant.now()", ts);
            ...
            Instant finalFechaRespuesta = fechaRespuesta;
            ...
            r.setTimestampRespuesta(fechaRespuesta);
